...
Peer.prototype.start = function () {
    ...
    var socketHandling = function (adr) {
        ...
        // unicast socket - now reusing socket across instances
        if (!socketMapUC[adr]) {      
            var uc = null;
            try {
            ...
            socketMapUC[adr] = socketMap[adr].unicast ;
        } else {
            socketMap[adr].unicast = socketMapUC[adr];
            var uc = socketMap[adr].unicast
            uc.on("message", onMessage);
            uc.on("listening", onListening);
            uc.on('error', onError);
            uc.on('close', onClose);
        }
		// multicast socket - now reusing socket across instances
        if (!socketMapMC[adr]) {
            var mc = null;
            try {
                // this is for version >= 0.12.x
            ...
            socketMapMC[adr] = socketMap[adr].multicast
        } else {
            socketMap[adr].multicast = socketMapMC[adr];
            var mc = socketMap[adr].multicast;
            mc.on("message", onMessage);
            mc.on("listening", onListening);
            mc.on('error', onError);
            mc.on('close', onClose);
